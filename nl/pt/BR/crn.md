...
Para serviços que são registrados no catálogo do {{site.data.keyword.Bluemix_notm}}, o
`service-name` deve corresponder a um dos serviços que estão registrados no serviço de Catálogo global do {{site.data.keyword.Bluemix_notm}}. É
a propriedade `name` retornada pela API `GET https://resource-catalog.bluemix.net/api/v1/{id}` do serviço de Catálogo global do {{site.data.keyword.Bluemix_notm}} para a instância de recurso correspondente ou o `service-name` exibido pela interface da linha de comandos: `ibmcloud service offerings` na coluna `service`.
