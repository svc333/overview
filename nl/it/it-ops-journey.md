...
Ottieni automaticamente la connettività a {{site.data.keyword.vpn_full}} quando viene configurato il tuo account {{site.data.keyword.cloud_notm}}. Per impostazione predefinita, il tuo server ha un indirizzo IP pubblico e uno privato. Se vuoi che il tuo server sia privato, puoi disattivare l'interfaccia pubblica dopo che è stato eseguito provisioning del tuo server oppure ordinare il tuo server come privato. Per ulteriori informazioni, vedi [Introduzione alla VPN (Virtual Private Networking)](/docs/infrastructure/iaas-vpn/getting-started.html).
