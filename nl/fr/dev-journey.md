 ...
 * Interface de ligne de commande (CLI) {{site.data.keyword.cloud_notm}}

Dans les portails de développeur {{site.data.keyword.cloud_notm}}, vous pouvez :
...
* Utiliser une [interface de ligne de commande](/docs/cli/index.html#overview) pour le développement local.
...
Nous souhaitons que vous puissiez facilement utiliser et personnaliser les kits de démarrage. Les kits de démarrage assemblent dans le langage de votre choix une application de production squelette prête pour le déploiement en cloud. Chaque kit de démarrage inclut un langage, une infrastructure et un modèle pour un cas d'utilisation spécifique et pour la réutilisation du code.
...
Vous pouvez commencer à partir du tableau de bord {{site.data.keyword.cloud_notm}} ou d'un kit de démarrage vide. Une fois que vous avez nommé votre application et que vous avez sélectionné un groupe de ressources, sélectionnez le point de démarrage [**Utilisation de votre propre code**](/docs/apps/tutorials/tutorial_byoc.html), indiquez l'URL du référentiel Git qui contient votre code puis cliquez sur **Créer**.
...
Utilisez l'interface de ligne de commande pour coder, générer et exécuter votre application localement. La pratique courante consiste à créer votre application à partir d'un portail de développeur dans la console {{site.data.keyword.cloud_notm}}, utiliser les outils de développeur pour effectuer un développement localement puis envoyer les mises à jour au référentiel et fusionner afin de démarrer votre chaîne d'outils de déploiement.
...
Les fonctions de développeur intégrées dans {{site.data.keyword.cloud_notm}} s'alignent sur ces ensembles de compétences et permettent à votre équipe d'utiliser une plateforme pour générer, fournir, exécuter et gérer votre application. Par exemple, un développeur numérique qui crée une application mobile peut utiliser le {{site.data.keyword.cloud_notm}} [portail de développeur mobile](https://{DomainName}/developer/mobile/dashboard){: new_window} ![Icône de lien externe](../icons/launch-glyph.svg "Icône de lien externe"), un générateur d'application cognitif peut utiliser le [portail de développeur Watson](https://{DomainName}/developer/watson/dashboard){: new_window} ![Icône de lien externe](../icons/launch-glyph.svg "Icône de lien externe") en même temps que [Watson Studio](https://{DomainName}/catalog/services/watson-studio){: new_window} ![Icône de lien externe](../icons/launch-glyph.svg "Icône de lien externe"), un développeur de flux peut utiliser [IBM Real-Time Analytics](/docs/services/StreamingAnalytics/index.html) et le [service de distribution continue d'{{site.data.keyword.cloud_notm}}](/docs/services/ContinuousDelivery/index.html) simplifie le travail d'un spécialiste DevOps.

Etes-vous prêt à commencer à générer des applications de qualité prêtes pour la production ? [Cliquez ici](/docs/apps/tutorials/tutorial_web.html) pour générer une application sur {{site.data.keyword.cloud_notm}} dès maintenant !
