...
독립 결함 도메인입니다. 구역은 내결함성을 개선하고 대기 시간을 감소시키는 데 도움이 되도록 디자인된 추상입니다.
...
공용 또는 개인용 컨테이너 이미지 저장 및 배포 서비스입니다. [컨테이너(container)](#x2010901), [이미지(image)](#x2024928)도 참조하십시오.
...
서버가 클라우드 환경 내에서 공용 인터넷 및 사설 서브넷과 통신할 수 있도록 일대일 NAT(Network Address Translation)를 이용하는 라우팅 가능한 공용 IP 주소입니다. 유동 IP 주소는 vNIC(Virtual Network Interface Card)를 통해 인스턴스(예: 가상 서버 인스턴스), 로드 밸런서 또는 VPN 게이트웨이와 연관되어 있습니다.
