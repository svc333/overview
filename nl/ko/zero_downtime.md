...
*지역(geography)*은 지리적 위치 또는 대형 정치 단체로서, 하나 이상의 지역(region)으로 구성됩니다. 일부 서비스는 여러 지역(region)을 횡단하고 지역(region) 간 데이터 복제 및 이동을 투명하게 관리하는 지역(geography) 레벨에서 지원됩니다. 예를 들어 {{site.data.keyword.cos_full_notm}}에 대한 EU 지역의 데이터는 EU 국가 내에 있는 여러 데이터 센터의 데이터를 복제할 수 있습니다.  
   * *메트로(metro)*는 특정 지리적 지역 내에 있는 특정 도시 이름입니다. 예를 들어 달라스는 북미 지역의 메트로입니다. 
      * *데이터 센터*는 클라우드 서비스를 제공하는 서버의 실제 위치입니다. 확장된 지역 내의 데이터 센터는 지역 이름과 번호를 사용하는 옵션으로 식별할 수 있습니다. 
      * *지역(region)*은 정의된 지리적 영역입니다. 지역은 특정 우편번호 영역, 구/군/시, 시/도, 시/도 그룹 또는 국가 그룹이 될 수 있습니다. 목록에서 데이터 센터와 지역(region)의 차이는 데이터 센터를 식별하는 숫자로 구별할 수 있습니다. 
         * *구역(zone)*은 결함 허용을 개선하고 대기 시간을 감소시키는 데 도움이 되도록 디자인된 독립된 결함 도메인입니다. 구역은 지역 이름과 번호로 구별할 수 있으며 지역 내에 중첩됩니다. 
...
|토론토 01       |tor01 |
|멕시코 01        |mex01 |
{: caption="표 1. 북미의 데이터 센터" caption-side="top"}
...
|------------------|---------|
|상파울루 01     |sao01   |
{: caption="표 2. 남미의 데이터 센터" caption-side="top"}
