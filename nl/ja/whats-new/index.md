...
### {{site.data.keyword.cloud_notm}} のグローバル・ロケーション名を更新
最新情報: 2018 年 11 月 1 日

{{site.data.keyword.cloud_notm}} はグローバルな利用可能拠点の拡大を継続しているため、弊社では、世界中の地理、地域、およびデータ・センターに関して、分かりやすく一貫した階層をより適切にサポートするためにロケーションの命名構造を更新します。現在のグローバル地域に精通しているお客様の場合、米国南部やシドニーなどの名前に見覚えがあるでしょう。弊社では、これらのロケーション名を、データ・センターが物理的に存在する都市の名前に合致させていきます。

現時点では、プログラマチック ID は変更されないため、 API の観点からは何の影響もありません。以下に、古いロケーション名と新しいロケーション名を示す表を示します。 データ・センターおよび地域の詳細情報および包括的リストについては、[サービスの可用性](docs/resources/services_region.html)を参照してください。

| 以前のロケーション表示名 | 新しいロケーション表示名 | コード |
|----------|---------|---------|
| 米国南部 | ダラス | us-south | 
...
{{site.data.keyword.cloud_notm}} Identity and Access Management (IAM) の使用により、アカウント内の他のユーザーが共通のアカウント管理タスクを実行できるようにすることができます。使用可能な 1 つまたはすべてのアカウント管理サービスでアクセス・ポリシーを作成することにより、ユーザーの招待と削除、アクセス・グループの管理、サービス ID の管理、プライベート・カタログ・サービスの保守、および請求処理のモニターと使用状況の追跡などの責務を簡単に委任することができます。アクセス・ポリシーをセットアップするには、以下の 4 つの個別のアカウント管理サービスとすべてのサービス・オプションを使用できます。

* ユーザーの招待と削除のためのユーザー管理
* アクセス権限の作成、編集、削除、更新、割り当てのための IAM アクセス・グループ 
* アカウント全体でサービス ID および関連する API キーへのアクセス権限を表示、作成、削除、および割り当てるための IAM ID サービス
* プライベート・カタログ・オファリングを表示し、オファリングのメタデータおよび可視性を更新するためのグローバル・リソース・カタログ
* 割り当てられた役割に基づいてそれぞれの個別のアカウント管理サービス・オプションにアクセスするため、および請求処理と使用状況の追跡にアクセスするためのすべてのアカウント管理サービス。

ポリシーがオンになっているアカウント管理サービス、および割り当てられている役割に基づいてユーザーが実行できるタスクについて詳しくは、[アカウント管理サービスのためのプラットフォーム管理の役割とアクションの例](/docs/iam/users_roles.html#platformrolestable2)を参照してください。
...
### dashDB for Transactions から {{site.data.keyword.DB2OnCloud_short}} に名前を変更しました
