...
災害復旧とは、単一ロケーションでの壊滅的な障害または可用性の消失が発生しても存続できるよう対処することに関する内容です。 災害復旧が確実に実施されるようにするには、複数のロケーションに複数の {{site.data.keyword.Bluemix_notm}} 環境をデプロイして、単一障害点を回避する必要があります。 これらの環境は、Public、Dedicated、または Local の各プラットフォームを組み合わせたものにすることができます。  
...
コンソールのリソース・リスト・ビューから、デフォルトですべてのロケーションのすべてのリソースを表示できます。 特定のロケーションのリソースを表示および処理する場合は、**「ロケーション」**メニューを展開し、リストからロケーションを選択します。 特定の地理的ロケーションを展開することにより、個々のデータ・センター、地域、またはゾーンを基準にフィルター操作できます。

![データ・センター、およびゾーンがある地域を含むメトロが含まれたジオグラフィーを示すロケーション階層](images/Location_hierarchy.svg)

例えば、ロンドン 2 (eu-gb-2) ゾーンにデプロイされたリソースがある場合、リソース・リストをフィルターに掛けて、それらのリソースのみを表示することができます。 ゾーンは地域内にあり、地域はメトロ・ロケーション別に編成されています。 リストをロンドン 2 (eu-gb-2) ゾーンでフィルターに掛けるには、**「ロンドン」**メトロ・オプションを展開し、次に**「ロンドン (eu-gb)」**地域オプションを展開します。 その地域内で、選択可能なゾーンのリストから選択できます。 特定のデータ・センターにデプロイされたリソースがある場合、例えば ロンドン 02 (lon02) のような固有のメトロ・ロケーションと英数字コードによって、そのデータ・センターを識別できます。

グローバルに配置されているリソースが存在する場合もあります。 **「グローバル」**オプションは、地域やゾーンに関係なく、グローバルにアクセス可能なサービスの論理インスタンスが 1 つのみ、顧客のアプリケーションに公開されていることを意味します。 このようなタイプのリソースは、グローバル・エンドポイントからアクセス可能です。
...
![利用可能なデータ・センターのマップ](images/Global-View.svg)
...
ダウン時間に対する請求を送信するには、[{{site.data.keyword.Bluemix_notm}} サポート](https://cloud.ibm.com/unifiedsupport/supportcenter){: new_window} ![外部リンク・アイコン](../icons/launch-glyph.svg "外部リンク・アイコン") までお問い合わせください。
...
| <99.95%                                                                       |<99.5%              |10%             |
| <99.90%                                                                       |<99.0%              |25%             |
{: caption="表 2. 月次可用性のサービス・レベル" caption-side="top"}
...
ダウン時間には、定期保守や告知済みの保守の時間は含まれません。 30 分間連続するダウン時間期間が発生するたびに、障害によって直接影響を受けた、特定されたサービスの月額料金の 5% に相当する額のクレジットを受け取ります。 ダウン時間が、連続する 30 分未満の場合は、クレジットを受け取ることはできません。 この計算に合うように、異なる障害タイプのダウン時間を合算することはできません。 
...
| > 14 時間                | 80%            |
| > 18 時間                | 80%            |
{: caption="表 3. ハードウェアの交換またはアップグレードの影響を受けるサービスの月額料金に基づくクレジット補償" caption-side="top"}
