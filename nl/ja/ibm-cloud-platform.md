...
開発者が {{site.data.keyword.Bluemix_notm}} を単に試してみようとしている場合は、直接カタログに移動し、探索したいオファリングの参照を開始して、自分のライト・アカウントに追加することができます。環境の準備が整い、実動でアプリを実行する準備ができたら、アカウント内に以下の基本的項目をセットアップすることを検討してください。
...
会社の財務担当にとっての関心事は、複数のチームおよび部門にわたって請求処理および使用量の管理をいかに単純化するかということです。サブスクリプション・アカウントを使用すると、{{site.data.keyword.Bluemix_notm}} エンタープライズを作成することができ、これにより、アカウントの一元管理、統合された請求処理、およびトップダウンの使用量レポートが提供されます。エンタープライズは、エンタープライズ・アカウント、アカウント・グループ、および個々のアカウントで構成されます。 
...
* スタンドアロン・アカウントと同じように、エンタープライズ内のアカウントには、リソース、リソース・グループ、Cloud Foundry 組織およびスペース、および独立したアクセス許可が含まれます。

詳しくは、『[エンタープライズをセットアップするためのベスト・プラクティス](/docs/account?topic=account-enterprise-best-practices)』を参照してください。
## 価格設定および請求処理
{: #pricing-billing}
