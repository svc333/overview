...
如果您是开发者，并且只是要试用 {{site.data.keyword.Bluemix_notm}}，您可以直接转至目录并开始浏览要探索的产品，然后将其添加到轻量帐户。当您已准备好开始使用环境并在生产环境中运行应用程序，请考虑在帐户中设置以下基本项：
...
作为公司的财务管理人员，您可能希望简化管理多个团队和部门的计费和使用情况的方式。通过预订帐户，可以创建 {{site.data.keyword.Bluemix_notm}} 企业，以提供集中帐户管理，合并计费和自顶向下使用情况报告。企业由企业帐户、帐户组和各个帐户组成。 

* 企业帐户是企业中其他所有帐户的父帐户。对整个企业的计费在企业帐户级别进行管理。
* 帐户组用于组织相关帐户。您可以在统一视图中查看帐户组中包含的所有帐户的资源使用成本。
* 与独立帐户类似，企业中的帐户包含资源和资源组、Cloud Foundry 组织和空间以及独立访问许可权。

有关更多信息，请参阅[设置企业的最佳实践](/docs/account?topic=account-enterprise-best-practices)。
## 定价和计费
{: #pricing-billing}
