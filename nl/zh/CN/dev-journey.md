...
# 探索 {{site.data.keyword.cloud_notm}} 中开发者的旅程
{: #dev-journey}
...
在 {{site.data.keyword.cloud_notm}} 开发者门户网站中，您可以：
...
通过入门模板工具包，我们希望您拥有易于使用且可定制的体验。入门模板工具包按您选择的语言来组合框架生产应用程序，可随时用于云部署。每个入门模板工具包中都包含针对特定用例的语言、框架和模式，并支持复用代码。
...
* 通常，**演示**的质量和精确度较高，并且会使用一系列服务和集成点。演示通常需要设置时间，并用于证明业务问题或演示平台功能。您可以将演示用于评估采用云的阶段。有时，演示代码是生产代码中所包含的代码。
* **样本**是由特定功能、特性、服务或用户旅程组成的一个简单示例。样本可以复用，也可以包含在生产应用程序中。样本通常用于展示技术功能，以及说明用于解决技术问题的可行方法。
...
通过 {{site.data.keyword.cloud_notm}} 入门模板工具包创建应用程序时，应用程序首先会具有一个经过验证的体系结构，该体系结构也会反映出所选语言的最佳实践。
...
{{site.data.keyword.cloud_notm}} 中内置的开发者功能与这些技能集相对应，并支持团队使用一个平台来生成、交付、运行和管理应用程序。例如，创建移动应用程序的数字开发者可以使用 {{site.data.keyword.cloud_notm}} [Mobile 开发者门户网站](https://{DomainName}/developer/mobile/dashboard){: new_window} ![外部链接图标](../icons/launch-glyph.svg "外部链接图标")，认知应用程序构建者可以将 [Watson 开发者门户网站](https://{DomainName}/developer/watson/dashboard){: new_window} ![外部链接图标](../icons/launch-glyph.svg "外部链接图标") 与 [Watson Studio](https://{DomainName}/catalog/services/watson-studio){: new_window} ![外部链接图标](../icons/launch-glyph.svg "外部链接图标") 一起使用，流开发者可以使用 [IBM Real-Time Analytics](/docs/services/StreamingAnalytics/index.html)，[{{site.data.keyword.cloud_notm}} Continuous Delivery 服务](/docs/services/ContinuousDelivery/index.html)则简化了 DevOps 专家的工作。

准备好开始构建高质量的生产就绪型应用程序了吗？[单击此处](/docs/apps/tutorials/tutorial_web.html)立即在 {{site.data.keyword.cloud_notm}} 上构建应用程序！
