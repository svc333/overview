...
如果您是開發人員，且只想試用 {{site.data.keyword.Bluemix_notm}}，可以直接進入型錄並開始瀏覽您想要探索的供應項目，然後新增至「精簡」帳戶。當您已準備好開始使用某個環境，並讓應用程式在正式作業中執行時，請考慮在您的帳戶中進行基本設定：
...
身為貴公司的財務主管，您可能有興趣簡化在多個團隊和部門之間管理計費和用量的方式。使用「訂閱」帳戶，您可以建立 {{site.data.keyword.Bluemix_notm}} 企業，以提供集中化的帳戶管理、合併計費，以及由上而下的用量報告。企業包含企業帳戶、帳戶群組及個別帳戶。 

* 企業帳戶是企業中所有其他帳戶的母帳戶。整個企業的計費是在企業帳戶層級進行管理。
* 帳戶群組提供一種組織相關帳戶的方式。另外，您可以統一檢視帳戶群組中所包含的所有帳戶之中的資源用量成本。
* 與獨立式帳戶類似，企業中的帳戶包含資源及資源群組、Cloud Foundry 組織和空間，以及獨立存取權。

如需相關資訊，請參閱[設定企業的最佳作法](/docs/account?topic=account-enterprise-best-practices)。
## 定價與計費
{: #pricing-billing}
