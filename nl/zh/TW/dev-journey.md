...
專案係含括了組成應用程式之程式碼、資料、服務及工具鏈等關聯項目。例如，{{site.data.keyword.cloud_notm}} 行動專案包含應用程式和後端邏輯、資料儲存、分析和安全服務等的程式碼，並已針對持續交付而設定。
...
專案包括一個 Readme 檔，其中包含專案的技術詳細資料，並解釋執行應用程式所需的項目（如果並非現成可用）。
{: tip}
...
## 指令行介面
{{site.data.keyword.dev_cli_notm}} 可讓您在本端編寫程式碼、建置及執行專案。一般模式是透過開發人員儀表板建立專案，並使用 {{site.data.keyword.dev_cli_notm}} 在本端開發，然後將更新項目推送至儲存庫，並合併以啟動部署工具鏈。
